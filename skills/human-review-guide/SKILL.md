...
Dispatch a **single subagent** to classify every chunk. Read `references/agent-prompts.md` § Triage Agent for the full prompt template.
...
Dispatch a **single subagent** that processes all DECIDE and READ chunks **sequentially** — not parallel, because chunks often relate to each other and the analysis benefits from accumulated context. Read `references/agent-prompts.md` § Deep Analysis Agent for the full prompt template.
